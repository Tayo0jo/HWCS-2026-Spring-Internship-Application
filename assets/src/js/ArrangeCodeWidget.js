...
export class ArrangeCodeWidget extends Widget {
  ...
  onDragStart = (event) => {
    ...
    this.draggedContainerIndex = this.getBlockContainers().indexOf(
      this.draggedContainer
    );
  };
  ...
  onDrop = (event) => {
    ...
    if (!this.draggedContainer || this.draggedContainer === dropContainer)
      return;
  ...
  getBlockContainers() {
    return Array.from(
      this.container.querySelectorAll(this.SELECTORS.BLOCK_CONTAINER)
    );
  }
  ...
  checkOrder(blockContainers) {
    for (let i = 0; i < blockContainers.length - 1; i++) {
      // Parse data-index as integers to avoid lexicographic ordering issues
      const firstAttr = blockContainers[i].getAttribute("data-index");
      const secondAttr = blockContainers[i + 1].getAttribute("data-index");
      const firstIndex = parseInt(firstAttr, 10);
      const secondIndex = parseInt(secondAttr, 10);

      // If either index is missing or not a number, treat the order as incorrect
